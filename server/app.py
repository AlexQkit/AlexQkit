...
def addGate():
    if request.method == 'POST':
        ...
        # receivedDictionary must contaians
        # {"gateName" : str, "listIndex" : int, "wires" : list of integers}
        c.addGate(receivedDictionary["gateName"],
                  receivedDictionary["listIndex"], receivedDictionary["wires"])
...
def draggableCircuit():
    if request.method == 'POST':
        ...
        try:
            ...
            r.setter(receivedDictionary["shots"], receivedDictionary["API_TOKEN"],
                     receivedDictionary["device"], circuit)
...
def addCustomGates():
    ...
    if isUnitary:
        ...
        c.gatesObjects[receivedDictionary["gateName"]] = f.matrixToGateObject(
            matrix, receivedDictionary["gateName"])
...
def subCircuitCustomGate():
    if request.method == 'POST':
        ...
        if isUnitary:
            c.gatesObjects[receivedDictionary["gateName"]] = f.matrixToGateObject(
                complexMatrix, receivedDictionary["gateName"])
        return jsonify({"isUnitary": isUnitary})
...
def nthRoot():
    if request.method == 'POST':
        ...
        c.gatesObjects[gateName] = f.matrixToGateObject(
            unitaryGate.to_matrix(), gateName)
...
def booleanExpress():
    if request.method == 'POST':
        ...
        if recievedData['indices'] != ['']:
            booleanCircut = BooleanFunction.buildBooleanCircuit_indecies(
                recievedData["vars"], recievedData['fn'], recievedData['indices'])
        else:
            booleanCircut = BooleanFunction.buildBooleanCircuit(
                recievedData["vars"], recievedData['fn'])
