...
###############################################################################################################################
    ...
    def setter(self, receivedDictionary):
        ...
        if receivedDictionary["repeated"] != {}:  # apply loops if exist
            ...
            self.cols = Repeat.repettion(
                receivedDictionary["rows"], receivedDictionary["repeated"]['listOfPos'], receivedDictionary["repeated"]['listOfRep'])
            ...
            # 2D list contains circuit gates
...
###############################################################################################################################
    ...
    def reversedMatrix(self, matrix, wires):
        ...
        for i in range(len(matrixCopy)):
            ...
            pos = int(''.join(
                reversed(str(("{0:0"+str(wires).replace('.0000', '')+"b}").format(i)))), 2)
            tempList = matrixCopy[pos]
            for j in range(len(tempList)):
                pos = int(''.join(
                    reversed(str(("{0:0"+str(wires).replace('.0000', '')+"b}").format(j)))), 2)
        ...
        if int(column[firstAppear][1:underscorePos]) == 1:  # one wire
            ...
            for i in range(firstAppear+1, len(column)):
                if column[firstAppear][:pointPos] == column[i][:pointPos]:
                    ...
                    # arrange the input wires according to the indices
                    pos.insert(index, i)
                    column[i] = "i"
    ...
    def controlsPos(self, column):
        ...
        for i in range(len(column)):
            if str(column[i]) == "●":
                ...
                # applying x gate before and after a closed control to convert it to an open control
                self.circuit.x(i)
                oc.append(i)
                c.append(i)
                column[i] = "i"
...
###############################################################################################################################
    ...
    def conditionalLoop(self, column, index):
        ...
        self.circuit = QuantumCircuit(self.num_qubits, self.num_qubits)
        self.circuit.initialize(newInitialization, list(
            range(self.num_qubits-1, -1, -1)))
...
###############################################################################################################################
    ...
    def createDraggableCircuit(self):
        ...
        for i in range(self.exeCount):  # stop at tracing line position
            ...
            # check if the column is a conditional loop column given as ["loop.0","loop.1","loop.*",...]
            if self.cols[i][0][0] == "l":
                self.conditionalLoop(self.cols[i], i)
            # check if the column contains controls
            elif "●" in self.cols[i] or "○" in self.cols[i]:
                # it will be checked in the frontend and receive a list of flags
...
###############################################################################################################################
    ...
    def addGateObject(self, gate, listIndex, wires):
        """
        adds a gate object into the circuit.data list

        Parameters
        ----------
        gate : qiskit.circuit.library.standard_gates (gate object)
            The gate that will be added to the circuit
        listIndex : int
            An index of circuit.data list
        wires : list
            A list of integers that identifies the qubits -that the gate will be applied on-
        """
        self.circuit.append(gate, wires)  # listNode creation
        listNode = self.circuit.data.pop()  # listNode extraction
        # adding listNode to the desired index
        self.circuit.data.insert(listIndex, listNode)

###############################################################################################################################

    def addGate(self, gateName, listIndex, wires):
        """
        ...
        ----------
        gateName : str
            The gate name that will be added to the circuit
        listIndex : int
            An index of circuit.data list
        wires : list
            A list of integers that identifies the qubits -that the gate will be applied on-

        gateName representation:
        ...
        -----------------------------------------------------------------------------------------------------
        |        gate        |           representation            |                 example                |
        -----------------------------------------------------------------------------------------------------
        | controlled gates   | <list of (○,●)>.<standardGateName>  | ●●●○.X   (4-controls on x gate)        |
        |--------------------|-------------------------------------|----------------------------------------|
        | custom gates       | c<num_qubits>_<userInputName>       | c3_u   (3-qubit custom gate called "u")|
        |--------------------|-------------------------------------|----------------------------------------|
        | standard gates     | <gateName>                          | X, Y, Z, H, S, T, ...                  |
        |--------------------|-------------------------------------|----------------------------------------|
        | gates with angle   | <gateName>(<angle>)                 | Rx(30)                                 |
        |--------------------|-------------------------------------|----------------------------------------|

        """
        # listIndex = listPositions.find((column,wires))  # need to change
        numOfControls = 0
        name = gateName
        ...
        # applying X before and after a closed-control to optain an open-control
        # ○ (open-control)  is equivalent to  X●X
        for i in range(numOfControls):
            if gateName[i] == "○":
                # adding x after the gate (listIndex+1)
                self.addGateObject(self.gatesObjects["X"], listIndex+1, [i])
        for i in range(numOfControls):
            if gateName[i] == "○":
                # adding x before the gate (listIndex)
                self.addGateObject(self.gatesObjects["X"], listIndex, [i])
